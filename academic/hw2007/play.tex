...
\todo{Sentance a mess!} The |transform f x| traversal recursively applies |transform f| to all the children of |x|, and applies |f| to the value |x| whose children have been replaced.
...
This code can be read: apply the subtraction rule where you can, and where you cannot, do nothing. Adding additional rules is easy. Take for example: |x + y = 2 * x where x == y|. Now we can add this new rule into our existing transformation:
...
The Scrap Your Boilerplate approach \cite{lammel:syb} (known as SYB) provides a top-down transformation named |everywhere'|. We describe this traversal, and our reasons for \textit{not} providing it, even though it could easily be defined. We instead provide |descend|, based on the |compos| operator \cite{bringert:compos}.
...
\todo{Still vague} The |descend f x| computation applies |f| to all the children of |x|. Unlike |everywhere'|, there is \textit{no recursion} within |descend|.
...
What are the preconditions on |r| such that \ignore|forall x `o` rewrite r x == transform (always r) x|? The answer is that the constructors on the right-hand side of |r| must not overlap with the constructors on the left-hand side.
...
This function returns lists of pairs |(x,f)| where |x| is an element of the data structure which would have been returned by |universe|, and |f| replaces the hole which |x| was removed from. It has the following properties:
...
Requiring each instance of the |Uniplate| class to implement ten separate methods would be an undue imposition. Instead, given a type specific instance for a \textit{single} auxiliary method, we can define \textit{all ten} operations generically, at the class level. The auxiliary is:
...
If we are willing to make use of \textit{multi-parameter type classes} \cite{jones:mptc} we can define a class |Biplate| with |biplate| as its sole method.
