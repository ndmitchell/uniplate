...
_C (n t_) =
        n x_ -> unit n <> _T t_1 x_1 <> ... <> _T t_n x_n
    where x_ {-" \mbox{ are fresh, and } "-} length t_ == length x_
...
The derivation rules are given in Figure \ref{fig:derive}. The |_D| function takes a data type declaration, and generates a function over that data type. The |_C| function generates an alternative for each constructor. The |_T| function applies type specific behaviour: either a type is the target type on which an instance is being defined; or a primitive such as |Int|; or a concrete type; or a free type variable.
