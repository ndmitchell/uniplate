...
\begin{comment}
To define a fold on a data type, each constructor must be represented -- giving boilerplate problems once more.
\end{comment}
...
We can define derivation rules for the |children_| and |context| functions, allowing the generic definition:

\ignore\begin{code}
instance Uniplate typename where
...
We model the derivation of an instance by describing a derivation from a data type, to a set of declarations. The declaration rules have three functional parameters: |(<>)|, |unit| and |target| -- by varying these parameters we derive either |children_| or |context| functions.
...
context x ns = fst ((_N typename) x ns)
\end{code}
