...
replaceChildren x = (children_,context)
\end{code}

Given a value |x|, the function |replaceChildren| returns a pair: |children_| - all the maximal proper substructures of the same type; |context| - a function to insert a new set of children into the existing value. The caller of |context| must ensure that the length of the list given to |context| is the same as the length of |children_|.
...
propId x = x == context children_
    where (children_,context) = replaceChildren x
\end{code}

\subsection{Operations in terms of |replaceChildren|}
\label{sec:using_replacechildren}

All ten operations of \S\ref{sec:use_play} can be defined in terms of |replaceChildren| very directly. We define five functions as examples.
...
The common pattern is to call |replaceChildren|, then perform some action on the current children, often calling |context| to create a modified value. Some of these definitions can be made more efficient -- something we return to in \S\ref{sec:optimise_everything}.
...
\subsection{Defining |replaceChildren|}
...
\subsection{Automated Derivation of |replaceChildren|}
...
If |alpha| is constant, there is another way to abstract away the |replaceType| argument, as the following example shows.
