...
The Scrap Your Boilerplate approach \citep{syb:lammel} provides a top-down traversal named |everywhere'|. We describe their traversal, how it could be represented in Uniplate, and our reasons for \textit{not} providing it. We instead provide |descend|, based on the |compos| operator \cite{compos}.

The |everywhere'| transformation applies a function to a value, then continues the transformation on all the children of the freshly generated values. Typically, the intention in a traversal is to apply the function to \textit{every node exactly once}. Unfortunately, |everywhere'| does not have this property. Consider the following example:

\todo{Got to here}

\begin{example}
f (Neg (Neg x)) = x
f (Neg (Add x y)) = Sub (Neg x) (Neg y)
\end{example}
