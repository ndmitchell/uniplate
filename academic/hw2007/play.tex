...
\item We compare the \textit{conciseness} of operations using our library, showing our approach leads to less boilerplate.
...
We define various queries and transformations, using the |Expr| type defined in the introduction as an example throughout. A \textit{query} is a function that takes a value, and returns some information extracted out of the value. A \textit{transformation} takes a value, and returns a modified version of the original value. A \textit{traversal} may be either a query or a transformation. All the traversals rely on the class |Uniplate|, an instance of which is assumed for |Expr|. The definition of this class and its instances are covered in \S\ref{sec:implement_play}.
...
Another common operation provided by many boilerplate removal systems \citep{lammel:syb}\todo{more citations} is a function that transforms every node in a value. We define as our standard transformation a bottom up traversal.

\begin{code}
transform :: Uniplate alpha => (alpha -> alpha) -> alpha -> alpha
\end{code}

The |transform| function starts by applying the function |f| to all the bottom-most nodes in the tree, then progressively works up to the root node. The condition on ordering is that before applying |f| to an expression, all its uniformly typed sub-expressions must have had |f| applied.
...
The Scrap Your Boilerplate approach \citep{syb:lammel} provides a top-down traversal named |everywhere'|. We describe their traversal, and our reasons for \textit{not} providing it, even though it would be trivial to write. We instead provide |descend|, based on the |compos| operator \citep{compos}.
...
This definition tries to apply the rule everywhere in a bottom-up manner. If at any point it makes a change, then the new value has the rewrite applied to it. Assuming the set of rules is convergent \todo{cite baader and nipkow}, there will be no difference between a top-down and bottom-up application of the rules. In any case, the function will only terminate once a normal form is found.
...
transformOn ::  Uniplate alpha => ReplaceType beta alpha -> (alpha -> alpha) -> beta -> beta
