...
\todo{Unsure these claim are true. Is the foldId equivalence true? Is there any good example for requiring a more complex fold?}
...
The common pattern is to call |replaceChildren|, then perform some action on the current children, before combining them back. Writing additional traversals with different properties can be done in a similar manner. The definition of |everything| can be improved -- something we return to in \S\ref{sec:optimise_everything}.
...
When writing a |Play| instance for a particular data type, |isTarget| returns True for that data type. We define how to produce a function for each |Data| type that is used, using the |_D| function:
...
The |Play| class takes a value of type |alpha|, and operates on its children of type |alpha|. What we now require is something that takes a value of type |beta|, but operates on the children of type |alpha| within it -- we call this class |Biplate|. Typically the type |beta| will be a container of |alpha|. We can extend our operations by specifying how to find the |alpha|'s within the |beta|'s, and then perform the standard |Uniplate| operations upon the |alpha| type. In the above example, |alpha = Expr|, and |beta = Stmt|.
...
\begin{code}
universeOn :: Play to => ReplaceType beta alpha -> beta -> [alpha]
