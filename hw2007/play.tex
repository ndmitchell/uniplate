...
Following bottom-up traversals, many boilerplate schemes would define a top-down traversal. Once two traversals have been defined, how should the user pick between them? Instead we focus on modelling traversals as rewrites.

To model the previous two actions as rewrites



We define a set of rewrites to be a set of directed equations, where the left side may introduce unique free variables, and the right side may refer to them. More formally:

\begin{code}
data Rewrite  =  Match :-> Match

data Match    =  Free  Int
              |  Ctor  String [Match]
\end{code}

The validity condition is that each number within a |Free| in the right hand side occurs at most once, and each on the left hand side must also be present on the right.


We model our traversals as rewrite rules.
...
This can be rewritten in Haskell as:

\begin{code}
Sub x y = Add x (Neg y)
\end{code}

If we consider this to be a directed rule, stating that the
...
After introducing queries, most boilerplate approaches would naturally move on to traversals, as we do. However, we have separate traversals into two different types -- action traversals,
