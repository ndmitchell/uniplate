...
While the |replaceChildren| function is sufficient to implement all methods, the class also contains one additional function, with a suitable default:

\begin{code}
getChildren :: alpha -> [alpha]
...
We have introduced |Play| which will operate on values of type |Expr|. Now lets imagine that our |Expr| type is merely the expression type in a language with statements:
...
Now given a value of type |Stmt| there is no easy way to perform queries and extractions upon the |Expr| type embedded in the |Stmt| type using |Play|. We present a solution named |PlayEx|. To take the example of extracting all the variables we can write:
...
Note that this code is now uses the multi-parameter type class |PlayEx|, and permits all expressions containing |Expr| to be used as the input type. This new |variables| function will work on |Expr| and |Stmt|, but also |[Expr]|, |(Expr,Maybe Stmt)| and many more types besides.
...
instance PlayEx Stmt Expr where
    replaceType (Assign n x) = ([x], \[x'] -> Assign n x')
    replaceType (Sequence xs) = (concatMap replaceType xs,
        where (gen,rep) = replaceType
\end{code}
\caption{|PlayEx| class and sample instances.}
\label{fig:playex}
\end{figure}
