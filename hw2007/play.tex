...
instance (Data a, Play b, Typeable a, Typeable b)
     => PlayEx a b where
...
collect_generate_self :: (Data on, Play with, Typeable on,
     Typeable with) => on -> CC with on
...
collect_generate :: (Data on, Play with, Typeable on,
     Typeable with) => on -> CC with on
...
\begin{code} % play
free :: Exp -> [String]
free = fold unions $ \e y -> case e of
    EAbs s x -> delete s y
    EVar s -> [s]
    x -> y
    where unions = nub . concat
\end{code}
...
\subsubsection{The |fresh| function}
