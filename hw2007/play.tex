...
\begin{code}
children (Add (Neg (Lit 1)) (Var "x")) = [Neg (Lit 1), Var "x"]
\end{code}
...
What are the preconditions on |f| such that |forall x `o` rewrite f x == transform f' x|? We can start by defining a more abstract representation of a rewrite rule:
...
Defining the ten operations of \S\ref{sec:use_play} in terms of |replaceChildren| is easy. We define the four most common as examples. All the definitions are short.
...
transformOn ::  Uniplate to => ReplaceType beta alpha ->
...
transformExpr  = transformOn  replaceTypeExpr
...
transformEx  :: Biplate beta to => (alpha -> alpha) -> beta -> beta
...
Uni.constFold = transformEx optimise
...
Uni.increase k = transformEx (incS k)
...
We view the library as finished. There may be some traversals we have overlooked, but we feel they are small in number.
