...
\begin{code}
traverse :: Uniplate alpha => (alpha -> alpha) -> alpha -> alpha
\end{code}
...
This function returns lists with two elements. The first element of each pair is an element of the data structure which would have been returned by |universe|. The other is the function which would replace the hole which the given expression was removed from. It has the properties:

\begin{code}
forall x `o` universe x == map fst (contexts x)
forall x `o` all (== x) [b a | (a,b) <- contexts x]
\end{code}
...
We present signatures for all our methods in Figure \ref{fig:play}, including several monadic variants. In our experience, the most commonly used operations are |universe| and |traverse|, followed by |traverseM| and |descend|.


\section{Implementing the |Uniplate| class}
\label{sec:implement_play}

Requiring each instance of the |Uniplate| class to implement nine separate methods would be an undue imposition. Instead we can implement all of these operations on top of just a single method:
...
Defining the nine operations of \S\ref{sec:use_play} in terms of |replaceChildren| is easy. We define the four most common as examples. All the definitions are short.

\begin{code}
universe :: Uniplate alpha => alpha -> [alpha]
universe x = x :
    concatMap universe $ fst $ replaceChildren x

traverse :: Uniplate alpha => (alpha -> alpha) -> alpha -> alpha
traverse f x = f $ generate $ map (traverse f) current
    where (current, generate) = replaceChildren x

traverseM :: (Monad m, Uniplate alpha) => (alpha -> m alpha) -> alpha -> m alpha
...
We define a set of operations, including |universeOn| and |traverseOn|, which take an additional argument relative to the standard |Uniplate| operators. We call this additional argument |replaceType|, which describes how to move from the containing type (|beta|) to the contained type (|alpha|).
...
The intuition for |replaceType| is that starting from |beta|, the function should return the highest children in the tree of type |alpha|. If |alpha == beta| the root node should be returned:
...
We can now define |universeOn| and |traverseOn|. Each takes a |replaceType| function as an argument:

\begin{code}
universeOn :: Uniplate to => ReplaceType beta alpha -> beta -> [alpha]
universeOn replaceType x =
    concatMap universe $ fst $ replaceType x

traverseOn ::  Uniplate to => ReplaceType beta alpha ->
...
In the worst case, this approach requires an instance for each container/contained pair. In reality few traversal pairs are actually needed. The restricted number of type classes also gives increased type safety; instance such as |Biplate Expr Stmt| do not exist, so this error can be eliminated.
...
instance (Typeable a, Uniplate a) => PlateAll Stm a where
    plateAll x =
        case x of
            Assign    a b    -> plate Assign    |-  a |+ b
            Sequence  a      -> plate Sequence  |+  a
...
Consider the |Expr| type. To infer |Uniplate Expr| we require an instance for |PlateAll Expr Expr|. But to infer this instance we require |Uniplate Expr| -- which we are in the process of inferring! \footnote{GHC has co-inductive or recursive dictionaries, which Hugs does not. To allow continuing compatibility with Hugs, and the use of fewer extensions, we require the user to write these explicitly for each type.}
...
\begin{code}
universe x = fold concat (:)
-- universe . traverse f == map (traverse f) . universe
-- does not hold!
\end{code}
...
A list is defined inductively as |data [alpha] = [] || alpha : [alpha]|. We can therefore define a |Uniplate| instance as:

\begin{code}
instance Uniplate [alpha] where
...
We can also define a |Biplate| instance as:

\begin{code}
instance Uniplate alpha => Biplate [alpha] alpha where
...
getChildren = snd . replaceChildren

concatCont []     rest  =  rest
concatCont (x:xs) rest  =  x (concatCont xs rest)
\end{code}

Now we only perform one reconstruction. We can do better using GHC's list fusion. The user of |universe| is likely to a list comprehension, which is a good consumer. We can make |concatCont| a good consumer, and |f| a good producer:
...
\subsubsection{Optimising |PlateData|}
...
SYB.zeroCount = everything (+) (0 `mkQ` f)
...
Uni.constFold = traverseEx optimise
...
The constant-folding operation is a bottom-up traversal, requiring subexpressions to have been replaced before patterning matching upon them. Compos only supports top-down traversals, requiring a small traversal in the middle. Uniplate and SYB both support bottom-up traversals.
\end{examplename}
...
Uni.increase k = traverseEx (incS k)
...
\begin{code}
Uni.salaryBill x = sum [s | S s <- universeEx x]
...
The Compos approach requires much more residual boilerplate than Uniplate, particularly for queries, bottom-up traversals and type signatures. The Compos approach also requires a GADT representation.

When compared to SYB, Uniplate seems much more similar. For queries, Uniplate is able to make use of list comprehensions, which produces shorter code and does not require encoding a manual fold over the items of interest. For traversals, typically both are able to use the same underlying operation, and the difference often boils down to the |mkT| to wrap the operation.
...
\item[Direct:] These instances use the direct combinators, defined as required.
