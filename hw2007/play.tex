...
\item The volume of the code, and the repeatitive nature, can lead to subtle bugs, which are easy to overlook.
...
\item We are able make use of the existing compiler support for deriving SYB traversals automatically, to provide automatic derivation of our class.
\item Writing additional traversals is trivial, within the reach of even casual users of the library.
\item We compare the relative performance of some traversal mechanisms, something that has been neglected in some previous papers.
...
We define various queries and traversals, using the \C{Expr} type defined in the introduction as an example throughout. All the traversals rely on the class \C{Play}, an instance of which is defined for \C{Expr}. This instance and definition are defined in \S\ref{sec:implementation}.

\subsection{Queries}

This library provides a single method for implementing queries, the \C{allOver} method.

\begin{code}
allOver :: Play on => on -> [on]
\end{code}

This function takes a data structure, and returns a list of all data structures of the appropriate type found within it. Given a tree, allOver returns all the root of the tree, and all it's subtrees at all levels. This can be used to quickly flatten a tree structure into a list, for quick analysis via list comprehensions, as is done above. For example:

\begin{code}
allOver (Add (Neg (Var "x")) (Val 12)) =
    [Add (Neg (Var "x")) (Val 12), Neg (Var "x"), Var "x", Val 12]
\end{code}

One use of this mechanism for querying was given in the introduction. For another example, consider the task of checking for division by a literal 0 -- as this will cause a runtime error.
...
Extra context could perhaps be given by printing some of the value that is being divided by zero, to help narrow down the error. Using the \C{allOver} function, all queries can be expressed very quickly and directly.

\subsection{Bottom Up Traversals}

Define traversals in terms of LHS context and RHS generate.

The bottom up traversal is by far the most useful type of traversal.

\begin{code}
mapUnder :: Play on => (on -> on) -> on -> on
\end{code}

Let's now try and write a basic optimiser for our little language. Negation of literals can be performed in advance:
...
Here the Play method being used is \C{mapUnder}. This applies the given function to all the children of an expression, before applying it to the parent. This can be thought of as bottom-up traversal of the data structure. For most traversals, \C{mapUnder} is the only method to use. The optimise code merely pattern matches on the negation of a literal, and replaces it with the literal. Using a bottom-up traversal means that |Neg (Neg (Val 1))| is reduced to 1, while top-down traversal would leave |Neg (Val -1)|.

Given the algebraic rule that |a + a == 2 * a|, this can be added easily:
...
In practice, a top-down traversal is only rarely necessary. To come up with an example where it is takes some effort! Here is one such example. Consider the mathematical rule |(-a) - (-b) = b - a|. The operational part can be written as:

\begin{code}
f (Sub (Neg a) (Neg b))  = Sub b a
f x                      = x
\end{code}

Writing |mapUnder f x| seems tempting, but is not correct. Consider |Sub (Neg (Neg (Val 1))) (Neg (Neg (Val 2)))|. With a bottom-up traversal this will be reduced to |Sub (Neg (Val -1)) (Neg (Val -1))|, as the \C{Sub} created by \C{f} will not be further analysed. There are three alternatives to this problem.

\begin{code}
solution1 = mapUnder f
    where  f (Sub (Neg a) (Neg b))  = f (Sub b a)
           f x                      = x
...
solution3 (Sub (Neg a) (Neg b)) = solution3 (Sub b a)
solution3 x = compos solution3 x
\end{code}

It is only necessary where context RHS overlaps with generated LHS.
