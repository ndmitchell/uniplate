...
The distinguishing feature of our libraries is that the children are defined in terms of their type -- while this makes the traversals and queries simpler, it does mean that the definition is dependent on the types of the constructors. In order to model the constructors it is sufficient to define combinators for each function.
 
 
 We can model the type of an operation with the following construction:
...
We first introduce |PlayOn|, a method for playing from a given starting type. Next we generalise this to |PlayEx|
